...
import FavoriteGameDetails from './FavoriteGameDetails'

export default function UserFavoritesFuture() {
    ...
    const getGames = () => {
        setLoading(true)
        axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/football/future/favorites`)
            .then(({data}) => {
                setGames(data.data)
                setLoading(false)
            })
                                ...
                                <div className="divide-y-[1px] divide-gray-300 border-t">
                                    <div className="gap-4 m-4 space-y-4 text-sm sm:space-y-0 sm:grid sm:grid-cols-2 lg:grid-cols-3">
                                        {games.map((game, index) => {
                                            return (
                                                <FavoriteGameDetails key={index} game={game} />
                                            )
                                        })}
                                    </div>
                                </div>
