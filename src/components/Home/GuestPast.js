...
export default function GuestPast() {
    ...
    const getGames = () => {
        setLoading(true)
        axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/football/past`)
            .then(({data}) => {
                setGames(data.data)
                setLoading(false)
            })
            ...
            <div className="mx-auto max-w-7xl">
                <div className="space-y-4">
                    <div className="md:py-2 md:mx-8">
                        {games.length > 0 &&
                            <div className="m-4 my-4 bg-white border border-gray-300 rounded-md shadow-md">
                                <div className="px-4 pt-4">{formatGameDate(games[0].game_date)}</div>
                                <div className="divide-y-[1px] divide-gray-300 border-t border-gray-300">
